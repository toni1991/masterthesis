...
Das Interface \mintinline{text}{MySystem} nutzt dabei die in Package \mintinline{text}{com.secondpackage} enthaltene Klasse \mintinline{text}{MySystemPart}. Bei der Verwendung einer Klasse aus dem selben Package ist kein zusätzliches \mintinline{java}{import}-Statement notwendig. Das Importieren geschieht dabei implizit über das Setzen des Packagenamens wie in Zeile 1 gezeigt.
...
Das Einbinden eines Copybooks ist jedoch auch innerhalb der \mintinline{cobolfree}{PROCEDURE DIVISION} möglich wie \autoref{copyFileInCode} und \autoref{copyInCode} darstellen. Somit wird es möglich Logik in Form von Code der wiederverwendet werden soll auszulagern und an verschiedenen Stellen zu nutzen.
