...
Ein Programm kann besteht dabei aus vier fest definierten \textit{DIVISION}s:
...
\begin{itemize}
    ...
    \item \mintinline{cobolfree}{DATA DIVISION} -- Diese \textit{DIVISION} beinhaltet die Definitionen von Daten. Dazu zählen Variablen oder auch Datei-Record-Definitionen.
    \item \mintinline{cobolfree}{PROCEDURE DIVISION} -- Innerhalb dieser \textit{DIVISION} befindet sich der ausführbare Code.
\end{itemize}

Eine \textit{DIVISION} -- außer der \mintinline{cobolfree}{IDENTIFICATION DIVISION} -- kann wiederum aus verschiedenen Sections bestehen, wobei diese nur innerhalb der \mintinline{cobolfree}{PROCEDURE DIVISION} frei definiert werden können.
