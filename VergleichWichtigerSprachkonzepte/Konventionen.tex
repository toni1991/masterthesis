...
So kann man den Namen einer \mintinline{cobolfree}{SECTION} oder eines Paragraph mit einem Präfix versehen und die darin genutzten Variablen mit demselbigen kennzeichnen. Beispielsweise sollte eine Variable \mintinline{cobolfree}{PC-100-VALUE} nur in der \mintinline{cobolfree}{SECTION} \mintinline{cobolfree}{PC-100-PROCESS} verwendet werden. 

Oft finden sich in bestehendem Code auch Präfixe welche die Art des Speichers -- z.B. \mintinline{cobolfree}{WS-} für \mintinline{cobolfree}{WORKING-STORAGE} -- kennzeichnen. Eine Benennung nach den genutzten Programmteilen ist jedoch vorzuziehen und sorgt für klarere Struktur und Lesbarkeit.


\subsection*{Schlüsselwörter}
In COBOL ist es möglich bestimmte Schlüsselwörter zu verkürzen. Dabei kann jedoch nicht beliebig gekürzt werden. Es sind lediglich weitere Schlüsselwörter mit selber Funktion definiert, die genutzt werden können. Ein Beispiel dafür ist die \mintinline{cobolfree}{PICTURE}-Anweisung, die auch als \mintinline{cobolfree}{PIC} geschrieben werden kann. Dabei handelt es sich zwar nicht um eine Konvention im eigentlichen Sinne, jedoch findet sich in der Praxis oftmals COBOL-Code welcher gekürzte Schlüsselwörter verwendet.
