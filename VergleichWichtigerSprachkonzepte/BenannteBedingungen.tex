\subsection{Benannte Bedingungen}\label{cobolSec88}
...
Zeile 15 des Programms illustriert einen weiteren Anwendungsfall der benannten Bedingungen. So lässt sich der Wert der eigentlichen Variable setzen, indem der bedingten Variable der Wahrheitswert \cob{TRUE} zugewiesen wird. Das Ergebnis dieser Zuweisung wird in der Ausgabe von \autoref{88_cobol_listing} in Zeile 3 dargestellt. Zu beachten ist hierbei, dass die meisten COBOL-Compiler nur das Setzen des Wertes \cob{TRUE} erlauben.

Dieses Verhalten kann dazu genutzt werden, um Variablen mit bestimmten Werten zu belegen, wie \autoref{88_cobol_value_set_listing} zeigt. Oftmals fungieren diese benannten Bedingungen auch als Schalter \bzw Flags, \dahe je nach Wert werden unterschiedliche Funktionalitäten ausgeführt.
...
Der Anwendungsfall, dass eine benannte Bedingung in COBOL verwendet wird, um bestimmte Werte zu setzen, lässt sich in Java am elegantesten über Aufzählungstypen, also \jav{enum}s, realisieren. Wie der Typ \jav{ErrorMessage} in \autoref{88_java_value_set_listing} zeigt, setzt jeder einzelne konstante Wert des Aufzählungstypen eine eigene Fehlernachricht, welche anschließend über die \jav{getMessage}-Funktion verfügbar ist.